...
import { AiFillHome, AiOutlineMessage, AiOutlineSearch } from "react-icons/ai";
import { BiLogOut, BiNetworkChart } from "react-icons/bi";
  ...
  return (
    <>
      {login == true ? (
        <div
          style={inputStyle}
          className="h-14 w-full fullnav fixed border-b-2 flex z-10 font-serif "
          onClick={handleclicksearch}
        >
          <div className="resdiv">
            <div className="h-full w-5/12 flex  items-center ml-10">
              <img className="h-11 w-20 mr-5" src={link} alt="" />
              {/* <div className='flex'> */}
              ...
              <input
                ...
                type="text"
                placeholder="Search"
                className="place h-9 w-72 pl-8 focus:outline-none  rounded-sm border-0 bg-navinput"
              />
          ...
          {matchingNames ? (
            <ul className="dropdown-list">
              {matchingNames?.map((item, index) => (
                ...
                >
                  {" "}
                  <img className="h-7 w-7 mr-3" src={item?.image} alt="" />{" "}
                  {item?.name}
                </li>
              ))}
            </ul>
          ...
          <div
            className={`flex justify-evenly w-5/12 items-center ${
              showDropdown ? "dropdownopen" : "dropdownclose"
            }`}
          >
            <Link
              to="/"
              style={linkStyle("link1")}
              onClick={() => handleLinkClick("link1")}
              className="flex-col mt-2 flex w-14 items-center "
            >
            ...
            <Link
              className="flex-col mt-2 flex items-center resflex"
        ...
        <>
          <div
            style={inputStyle}
            className="h-14 w-full fixed border-b-2 flex justify-between  mr-3 items-center z-10 font-serif mobile-nav"
          >
            <img src={linkk} className="h-12 w-50 img-mobile " alt="" />
            <div className=" flex justify-center items-center m1-div">
              <Link
                to={"/register"}
                className="h-10 w-28 flex items-center justify-center mr-5  rounded-md"
              >
                Join Now
              </Link>
              <Link
                to={"/login"}
                className="h-10 w-28 border-2 flex items-center justify-center border-slate-500 rounded-md ml-mobile"
              >
                Sign In
              </Link>
            </div>
